(synth-fun verifier.error ((x0 Bool) (x1 Bool) (x2 Bool) ) Bool)
(synth-fun mainventry ((x0 Int) (x1 Int) (x2 Int) ) Bool)
(synth-fun mainv.lr.ph ((x0 Int) (x1 Int) (x2 Int) (x3 Int) (x4 Int) ) Bool)
(synth-fun mainv._crit_edge ((x0 Int) (x1 Int) (x2 Int) ) Bool)
(synth-fun mainv_17 ((x0 Int) (x1 Int) ) Bool)
(synth-fun mainvverifier.error ((x0 Int) ) Bool)
(synth-fun mainvverifier.error.split () Bool)
(synth-fun mainventry!_BAD () Bool)
(synth-fun mainv._crit_edge!_Cond ((x0 Int) (x1 Int) (x2 Int) ) Bool)
(synth-fun mainv._crit_edge!_FwdCond ((x0 Int) (x1 Int) (x2 Int) ) Bool)
(declare-var mainv%_20_0 Bool )
(declare-var mainv%_18_0 Int )
(declare-var vg_0 Int )
(declare-var mainv%_15_0 Int )
(declare-var vfind_condition_0 Int )
(declare-var mainv%_16_0 Bool )
(declare-var mainv%_14_0 Bool )
(declare-var mainv%_11_0 Bool )
(declare-var mainv%.1.i2_2 Int )
(declare-var mainv%.01.i1_2 Int )
(declare-var mainv%_0_0 Bool )
(declare-var mainv%_1_0 Int )
(declare-var vnd_0 Int )
(declare-var mainv%_3_0 Int )
(declare-var mainv%_4_0 Int )
(declare-var mainv%_5_0 Int )
(declare-var mainv%_8_0 Bool )
(declare-var mainventry_0 Bool )
(declare-var mainv%loop.bound_0 Int )
(declare-var mainv%_2_0 Int )
(declare-var mainv%_7_0 Int )
(declare-var mainv_9_0 Bool )
(declare-var mainv_10_0 Bool )
(declare-var |tuple(mainventry_0, mainv_10_0)| Bool )
(declare-var mainv%.0.i_0 Int )
(declare-var mainv%.0.i_1 Int )
(declare-var mainv%.0.i_2 Int )
(declare-var mainv.lr.ph_0 Bool )
(declare-var mainv%.1.i2_0 Int )
(declare-var mainv%.01.i1_0 Int )
(declare-var mainv%.1.i2_1 Int )
(declare-var mainv%.01.i1_1 Int )
(declare-var mainv._crit_edge_0 Bool )
(declare-var mainv%.1.i.lcssa_0 Int )
(declare-var mainv%.1.i.lcssa_1 Int )
(declare-var mainv%_12_0 Int )
(declare-var mainv%_13_0 Int )
(declare-var mainv.lr.ph_1 Bool )
(declare-var mainv_17_0 Bool )
(declare-var mainvverifier.error_0 Bool )
(declare-var mainv%.2.i_0 Int )
(declare-var mainv%.2.i_1 Int )
(declare-var mainv%_19_0 Int )
(declare-var mainvverifier.error.split_0 Bool )
(constraint (verifier.error false false false))
(constraint (verifier.error false true true))
(constraint (verifier.error true false true))
(constraint (verifier.error true true true))
(constraint (mainventry vg_0 vfind_condition_0 vnd_0))
(constraint (let ((a!1 (and (mainventry vg_0 vfind_condition_0 vnd_0)
                true
                (= mainv%_0_0 (= mainv%loop.bound_0 3))
                mainv%_0_0
                (= mainv%_1_0 vnd_0)
                (= mainv%_3_0 vnd_0)
                (= mainv%_5_0 vnd_0)
                (= mainv%_7_0 (+ mainv%_4_0 mainv%_2_0))
                (= mainv%_8_0 (> mainv%_7_0 8))
                (=> mainv_9_0 (and mainv_9_0 mainventry_0))
                (=> (and mainv_9_0 mainventry_0) mainv%_8_0)
                (=> |tuple(mainventry_0, mainv_10_0)| mainventry_0)
                (=> mainv_10_0
                    (or (and mainv_10_0 mainv_9_0)
                        |tuple(mainventry_0, mainv_10_0)|))
                (=> |tuple(mainventry_0, mainv_10_0)| (not mainv%_8_0))
                (=> (and mainv_10_0 mainv_9_0) (= mainv%.0.i_0 mainv%_7_0))
                (=> |tuple(mainventry_0, mainv_10_0)| (= mainv%.0.i_1 9))
                (=> (and mainv_10_0 mainv_9_0) (= mainv%.0.i_2 mainv%.0.i_0))
                (=> |tuple(mainventry_0, mainv_10_0)|
                    (= mainv%.0.i_2 mainv%.0.i_1))
                (=> mainv_10_0 (= mainv%_11_0 (> mainv%_2_0 0)))
                (=> mainv.lr.ph_0 (and mainv.lr.ph_0 mainv_10_0))
                (=> (and mainv.lr.ph_0 mainv_10_0) mainv%_11_0)
                (=> (and mainv.lr.ph_0 mainv_10_0)
                    (= mainv%.1.i2_0 mainv%.0.i_2))
                (=> (and mainv.lr.ph_0 mainv_10_0)
                    (= mainv%.01.i1_0 mainv%_2_0))
                (=> (and mainv.lr.ph_0 mainv_10_0)
                    (= mainv%.1.i2_1 mainv%.1.i2_0))
                (=> (and mainv.lr.ph_0 mainv_10_0)
                    (= mainv%.01.i1_1 mainv%.01.i1_0))
                mainv.lr.ph_0)))
  (=> a!1
      (mainv.lr.ph vg_0
                   vfind_condition_0
                   mainv%.1.i2_1
                   mainv%.01.i1_1
                   mainv%loop.bound_0))))
(constraint (let ((a!1 (and (mainventry vg_0 vfind_condition_0 vnd_0)
                true
                (= mainv%_0_0 (= mainv%loop.bound_0 3))
                mainv%_0_0
                (= mainv%_1_0 vnd_0)
                (= mainv%_3_0 vnd_0)
                (= mainv%_5_0 vnd_0)
                (= mainv%_7_0 (+ mainv%_4_0 mainv%_2_0))
                (= mainv%_8_0 (> mainv%_7_0 8))
                (=> mainv_9_0 (and mainv_9_0 mainventry_0))
                (=> (and mainv_9_0 mainventry_0) mainv%_8_0)
                (=> |tuple(mainventry_0, mainv_10_0)| mainventry_0)
                (=> mainv_10_0
                    (or (and mainv_10_0 mainv_9_0)
                        |tuple(mainventry_0, mainv_10_0)|))
                (=> |tuple(mainventry_0, mainv_10_0)| (not mainv%_8_0))
                (=> (and mainv_10_0 mainv_9_0) (= mainv%.0.i_0 mainv%_7_0))
                (=> |tuple(mainventry_0, mainv_10_0)| (= mainv%.0.i_1 9))
                (=> (and mainv_10_0 mainv_9_0) (= mainv%.0.i_2 mainv%.0.i_0))
                (=> |tuple(mainventry_0, mainv_10_0)|
                    (= mainv%.0.i_2 mainv%.0.i_1))
                (=> mainv_10_0 (= mainv%_11_0 (> mainv%_2_0 0)))
                (=> mainv._crit_edge_0 (and mainv._crit_edge_0 mainv_10_0))
                (=> (and mainv._crit_edge_0 mainv_10_0) (not mainv%_11_0))
                (=> (and mainv._crit_edge_0 mainv_10_0)
                    (= mainv%.1.i.lcssa_0 mainv%.0.i_2))
                (=> (and mainv._crit_edge_0 mainv_10_0)
                    (= mainv%.1.i.lcssa_1 mainv%.1.i.lcssa_0))
                mainv._crit_edge_0)))
  (=> a!1 (mainv._crit_edge vg_0 mainv%.1.i.lcssa_1 vfind_condition_0))))
(constraint (=> (and (mainv.lr.ph vg_0
                      vfind_condition_0
                      mainv%.1.i2_0
                      mainv%.01.i1_0
                      mainv%loop.bound_0)
         true
         (= mainv%_12_0 (+ mainv%.1.i2_0 mainv%.01.i1_0))
         (= mainv%_13_0 (+ mainv%.01.i1_0 (- 3)))
         (= mainv%_14_0 (> mainv%.01.i1_0 mainv%loop.bound_0))
         (=> mainv.lr.ph_1 (and mainv.lr.ph_1 mainv.lr.ph_0))
         (=> (and mainv.lr.ph_1 mainv.lr.ph_0) mainv%_14_0)
         (=> (and mainv.lr.ph_1 mainv.lr.ph_0) (= mainv%.1.i2_1 mainv%_12_0))
         (=> (and mainv.lr.ph_1 mainv.lr.ph_0) (= mainv%.01.i1_1 mainv%_13_0))
         (=> (and mainv.lr.ph_1 mainv.lr.ph_0) (= mainv%.1.i2_2 mainv%.1.i2_1))
         (=> (and mainv.lr.ph_1 mainv.lr.ph_0)
             (= mainv%.01.i1_2 mainv%.01.i1_1))
         mainv.lr.ph_1)
    (mainv.lr.ph vg_0
                 vfind_condition_0
                 mainv%.1.i2_2
                 mainv%.01.i1_2
                 mainv%loop.bound_0)))
(constraint (=> (and (mainv.lr.ph vg_0
                      vfind_condition_0
                      mainv%.1.i2_0
                      mainv%.01.i1_0
                      mainv%loop.bound_0)
         true
         (= mainv%_12_0 (+ mainv%.1.i2_0 mainv%.01.i1_0))
         (= mainv%_13_0 (+ mainv%.01.i1_0 (- 3)))
         (= mainv%_14_0 (> mainv%.01.i1_0 mainv%loop.bound_0))
         (=> mainv._crit_edge_0 (and mainv._crit_edge_0 mainv.lr.ph_0))
         (=> (and mainv._crit_edge_0 mainv.lr.ph_0) (not mainv%_14_0))
         (=> (and mainv._crit_edge_0 mainv.lr.ph_0)
             (= mainv%.1.i.lcssa_0 mainv%_12_0))
         (=> (and mainv._crit_edge_0 mainv.lr.ph_0)
             (= mainv%.1.i.lcssa_1 mainv%.1.i.lcssa_0))
         mainv._crit_edge_0)
    (mainv._crit_edge vg_0 mainv%.1.i.lcssa_1 vfind_condition_0)))
(constraint (=> (and (mainv_17 vg_0 mainv%.1.i.lcssa_0)
         true
         (= mainv%_18_0 vg_0)
         (= mainv%_19_0 (+ mainv%.1.i.lcssa_0 (- 1)))
         (=> mainvverifier.error_0 (and mainvverifier.error_0 mainv_17_0))
         (=> (and mainvverifier.error_0 mainv_17_0)
             (= mainv%.2.i_0 mainv%_19_0))
         (=> (and mainvverifier.error_0 mainv_17_0)
             (= mainv%.2.i_1 mainv%.2.i_0))
         mainvverifier.error_0)
    (mainvverifier.error mainv%.2.i_1)))
(constraint (=> (and (mainvverifier.error mainv%.2.i_0)
         true
         (= mainv%_20_0 (> mainv%.2.i_0 8))
         (not mainv%_20_0)
         (=> mainvverifier.error.split_0
             (and mainvverifier.error.split_0 mainvverifier.error_0))
         mainvverifier.error.split_0)
    mainvverifier.error.split))
(constraint (=> (and (mainv._crit_edge!_FwdCond vg_0 mainv%.1.i.lcssa_0 vfind_condition_0)
         (mainv._crit_edge vg_0 mainv%.1.i.lcssa_0 vfind_condition_0)
         true
         (= mainv%_15_0 vfind_condition_0)
         (=> mainv_17_0 (and mainv_17_0 mainv._crit_edge_0))
         (=> (and mainv_17_0 mainv._crit_edge_0) mainv%_16_0)
         mainv_17_0)
    (mainv_17 vg_0 mainv%.1.i.lcssa_0)))
(constraint (=> (and (not (mainv._crit_edge!_FwdCond
                vg_0
                mainv%.1.i.lcssa_0
                vfind_condition_0))
         (mainv._crit_edge vg_0 mainv%.1.i.lcssa_0 vfind_condition_0)
         true
         (= mainv%_15_0 vfind_condition_0)
         (=> mainvverifier.error_0
             (and mainvverifier.error_0 mainv._crit_edge_0))
         (=> (and mainvverifier.error_0 mainv._crit_edge_0) (not mainv%_16_0))
         (=> (and mainvverifier.error_0 mainv._crit_edge_0)
             (= mainv%.2.i_0 mainv%.1.i.lcssa_0))
         (=> (and mainvverifier.error_0 mainv._crit_edge_0)
             (= mainv%.2.i_1 mainv%.2.i_0))
         mainvverifier.error_0)
    (mainvverifier.error mainv%.2.i_1)))
(constraint (=> mainvverifier.error.split false))
(check-synth)
