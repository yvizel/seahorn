...
echo "cvc4 command: cvc4 nonhorn.sl --tlimit=$4" >>"$2"
for file in "$1"/**/*.sl; do
  sed -i 's/@/v/g' "$file"
  file_without_prefix="${file#$1}"
  echo "making directory $3/${file_without_prefix%/*}"
  mkdir -p "$3/${file_without_prefix%/*}"
  { time cvc4 "$file" "--tlimit=$4" >"$3/${file_without_prefix%%.*}.cvc4.res" 2>"$3/${file_without_prefix%%.*}.cvc4.out"; } 2>"$3/${file_without_prefix%%.*}.cvc4.time"
done
