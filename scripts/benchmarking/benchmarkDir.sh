...
if mkdir "$new_dir_name" && ./frontend.sh "$1" "$new_dir_name/settings.out" "$new_dir_name/reverseSmt2" "$new_dir_name/forwardSl"; then
  { [[ "$3" == "cosyn" ]] || [[ "$3" == "all" ]] ;} && ./runCosyn.sh "$new_dir_name/reverseSmt2" "$new_dir_name/settings.out" "$new_dir_name/cosyn"
  { [[ "$3" == "cvc4" ]] || [[ "$3" == "all" ]] ;} &&  ./runCVC4.sh "$new_dir_name/forwardSl" "$new_dir_name/settings.out" "$new_dir_name/cvc4"
fi
