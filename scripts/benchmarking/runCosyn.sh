...
for file in "$1"/**/*.smt2; do
    ...
    { time z3 "$file" -T:"$4" -v:1 fp.xform.slice=false fp.xform.inline_linear=false fp.xform.inline_eager=false > "$3/${file_without_prefix%%.*}.cosyn.res" 2> "$3/${file_without_prefix%%.*}.cosyn.out" ; } 2> "$3/${file_without_prefix%%.*}.cosyn.time"
done
