...
namespace seahorn
{
  ...
  void HornUnroll::solve()
  {
    HornifyModule &hm = getAnalysis<HornifyModule> ();
    ufo::ZSolver<EZ3> solver(hm.getZContext());

    // Expr exp = mk<TRUE>(m_pUnrolledDB->getExprFactory());

    // query and entry are FAPP(FDECL(...))
    // relations are FDECL(...)
    // cannot assert FDECL, can only assert FAPP, AND, IMPL etc.

    Expr query = m_pUnrolledDB->getQueries()[0];
    LOG("horn-unroll", llvm::errs ()<<"=== solver query: "<<*query<<"===\n";);
    solver.assertExpr(query);


    // if (isOpX<FDECL>(q))
    //   llvm::errs()<<"=== *** query is FDECL\n";
    // else if (isOpX<FAPP>(q))
    //   llvm::errs()<<"=== *** query is FAPP\n";

    // if (bind::IsConst()(q))
    //   llvm::errs()<<"=== *** query is Const\n";

    HornClauseDBCallGraph cg(*m_pUnrolledDB);
    m_pUnrolledDB->buildIndexes();
    cg.buildCallGraph();
    Expr entry = bind::fapp(cg.entry());
    LOG("horn-unroll", llvm::errs ()<<"=== solver entry: "<<*entry<<"===\n";);
    
    solver.assertExpr(entry);

    ExprVector worklist;
    ExprSet completed;
    ExprSet all_vars = {query, entry};

    worklist.push_back(query->left());
    completed.insert(entry->left());
    while (!worklist.empty())
    {
      Expr rel = worklist.back();
      worklist.pop_back();
      completed.insert(rel);

      Expr cond = mk<FALSE>(m_pUnrolledDB->getExprFactory());
      for (HornRule *rule : m_pUnrolledDB->def(rel))
      {
        cond = boolop::lor(cond, rule->body());
        Expr next_rel;
        if (isOpX<AND>(rule->body()))
        {
          next_rel = rule->body()->left()->left();
        }
        else
        {
          next_rel = rule->body()->left(); 
        }
        assert(isOpX<FDECL>(next_rel));
        if (completed.count(next_rel) == 0)
        {
          worklist.push_back(next_rel);
        }
      }
      Expr assertion = boolop::limp(bind::fapp(rel), cond);
      solver.assertExpr(assertion);

      extract_vars(all_vars, assertion);
    ...
    if (res) 
    {
      LOG("horn-unroll", errs()<<"=== SAT ===\n";);
      ufo::ZModel<EZ3> m = solver.getModel();
      LOG("horn-unroll", errs() << m <<"\n";);
    }
    else if (!res) LOG("horn-unroll", errs()<<"=== UNSAT ===\n";);
    else LOG("horn-unroll", errs()<<"=== UNKNOWN ===\n";);
