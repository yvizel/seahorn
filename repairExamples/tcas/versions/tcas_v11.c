...
bool Own_Below_Threat()
{
	if (find_condition()){ //Own_Tracked_Alt < Other_Tracked_Alt
		g();
		return 1;
	} else {
		return 0;
...
bool Own_Above_Threat()
{
	if (find_condition()){ //Other_Tracked_Alt < Own_Tracked_Alt
		g();
		return 1;
	} else {
		return 0;
...
int AllRepair_correct_alt_sep_test()
{
  ...
  // removed (need_upward_RA && need_downward_RA) case, which is unreachable if the rest of the code is correct
  if (need_upward_RA)
